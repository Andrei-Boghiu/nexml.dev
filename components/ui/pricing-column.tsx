        ...
        <section className="flex flex-col gap-3">
          {showDiscounts && (
            <div className="flex h-6 items-baseline gap-1">
              {originalPrice && (
                <span className="text-muted-foreground text-lg font-medium line-through">${originalPrice}</span>
              )}
            </div>
          )}
            ...
            <div className="h-6">
              {discountCode && (
                <p className="text-brand-foreground text-sm font-medium">
                  {discountPercentage}% off with code <Badge variant="brand-secondary">{discountCode}</Badge>
                </p>
              )}
            </div>
          )}
